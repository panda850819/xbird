...
function resolveFeaturesCachePath(): string {
  const override = process.env.XBIRD_FEATURES_CACHE ?? process.env.XBIRD_FEATURES_PATH;
  if (override && override.trim().length > 0) {
    return path.resolve(override.trim());
  }
  return path.join(homedir(), '.config', 'xbird', DEFAULT_CACHE_FILENAME);
}
...
function readOverridesFromEnv(): NormalizedFeatureOverrides | null {
  const raw = process.env.XBIRD_FEATURES_JSON;
