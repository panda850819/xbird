...
export function resolvePackageVersion(importMetaUrl?: string): string {
  const injected = typeof process === 'undefined' ? '' : (process.env.XBIRD_VERSION ?? '').trim();
...
function resolveNearestPackageRoot(startDir: string): string | null {
  let dir = startDir;
  for (let i = 0; i < 10; i += 1) {
    if (readPackageVersionFromJsonFile(path.join(dir, 'package.json'))) {
      return dir;
    }
    const parent = path.dirname(dir);
    if (parent === dir) {
...
export function resolveGitSha(importMetaUrl?: string): string | null {
  const injected = typeof process === 'undefined' ? '' : (process.env.XBIRD_GIT_SHA ?? '').trim();
  if (injected.length > 0) {
    return truncateSha(injected);
  }

  let dir = resolveStartDir(importMetaUrl);
  const packageRoot = resolveNearestPackageRoot(dir);
  for (let i = 0; i < 10; i += 1) {
    ...
    if (packageRoot && dir === packageRoot) {
      break;
    }
    const parent = path.dirname(dir);
    if (parent === dir) {
