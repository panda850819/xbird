...
export function registerUserTweetsCommand(program: Command, ctx: CliContext): void {
  ...
  program
    ...
    .argument('<handle>', 'Username to fetch tweets from (e.g., @example or example)')
    ...
    .addHelpText(
      ...
      () =>
        `\n${ctx.colors.section('Command Examples')}\n${[
          formatExample('xbird user-tweets @example', 'Get recent tweets from a user'),
          formatExample('xbird user-tweets example -n 10', 'Get 10 tweets (@ is optional)'),
          formatExample('xbird user-tweets @example -n 50', 'Fetch 50 tweets (paged)'),
          formatExample('xbird user-tweets @example --max-pages 3 -n 200', 'Safety cap (max 3 pages)'),
          formatExample('xbird user-tweets @example --json', 'Output as JSON'),
          formatExample('xbird user-tweets @example --cursor "DAABCg..."', 'Resume from cursor'),
        ].join('\n')}`,
    )
    .action(
      async (
