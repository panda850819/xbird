...
export function createProgram(ctx: CliContext): Command {
  ...
  program.addHelpText(
    'beforeAll',
    () =>
      `${ctx.colors.banner('xbird')} ${ctx.colors.muted(getCliVersion())} ${ctx.colors.subtitle(
        '— unofficial, script-friendly X CLI',
      )}`,
  );

  program
    .name('xbird')
    .description('Read, search, publish, and organize X posts via the web GraphQL API')
  ...
  program.addHelpText(
    ...
    () =>
      `\n${ctx.colors.section('Examples')}\n${[
        formatExample('xbird whoami', 'Show the logged-in account via GraphQL cookies'),
        formatExample('xbird --firefox-profile default-release whoami', 'Use Firefox profile cookies'),
        formatExample('xbird tweet "hello from xbird"', 'Send a tweet'),
        formatExample(
  ...
  program.addHelpText(
    ...
    () =>
      `\n\n${ctx.colors.section('Config')}\n${ctx.colors.muted(
        `  Reads ${ctx.colors.argument('~/.config/xbird/config.json5')} and ${ctx.colors.argument('./.xbirdrc.json5')} (JSON5)`,
      )}\n${ctx.colors.muted(
