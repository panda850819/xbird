...
function readConfigFile(path: string, warn: (message: string) => void): Partial<XbirdConfig> {
  if (!existsSync(path)) {
    return {};
  }
  try {
    const raw = readFileSync(path, 'utf8');
    const parsed = JSON5.parse(raw) as Partial<XbirdConfig>;
    return parsed ?? {};
  } catch (error) {
...
function loadConfig(warn: (message: string) => void): XbirdConfig {
  const globalPath = join(homedir(), '.config', 'xbird', 'config.json5');
  const localPath = join(process.cwd(), '.xbirdrc.json5');
...
export function createCliContext(normalizedArgs: string[], env: NodeJS.ProcessEnv = process.env): CliContext {
  ...
  function resolveTimeoutFromOptions(options: { timeout?: string | number }): number | undefined {
    return resolveTimeoutMs(options.timeout, config.timeoutMs, env.XBIRD_TIMEOUT_MS);
  }

  function resolveCookieTimeoutFromOptions(options: { cookieTimeout?: string | number }): number | undefined {
    return resolveTimeoutMs(options.cookieTimeout, config.cookieTimeoutMs, env.XBIRD_COOKIE_TIMEOUT_MS);
  }

  function resolveQuoteDepthFromOptions(options: { quoteDepth?: string | number }): number | undefined {
    return resolveQuoteDepth(options.quoteDepth, config.quoteDepth, env.XBIRD_QUOTE_DEPTH);
  }
