...
function withTempDir<T>(fn: (dir: string) => T): T {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'xbird-version-test-'));
  try {
    return fn(dir);
  } finally {
...
describe('getCliVersion', () => {
  afterEach(() => {
    delete process.env.XBIRD_VERSION;
    delete process.env.XBIRD_GIT_SHA;
  });
  ...
  it('formats injected version + sha', () => {
    process.env.XBIRD_VERSION = '9.9.9';
    process.env.XBIRD_GIT_SHA = 'abcdef123456';
  ...
  it('does not inherit the consumer repository sha from an installed package', () => {
    withTempDir((dir) => {
      const gitDir = path.join(dir, '.git');
      fs.mkdirSync(gitDir, { recursive: true });
      fs.writeFileSync(path.join(gitDir, 'HEAD'), 'eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee\n');

      const packageRoot = path.join(dir, 'node_modules', '@panda850819', 'xbird');
