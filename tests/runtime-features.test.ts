...
describe('runtime-features', () => {
  afterEach(() => {
    delete process.env.XBIRD_FEATURES_JSON;
    delete process.env.XBIRD_FEATURES_CACHE;
    delete process.env.XBIRD_FEATURES_PATH;
    clearFeatureOverridesCache();
  });

  it('applies global and set overrides from env json', () => {
    process.env.XBIRD_FEATURES_JSON = JSON.stringify({
      global: {
        global_flag: true,
        ignored: 'nope',
      },
  ...
  it('refresh merges defaults with existing cache', async () => {
    const cacheDir = path.join(os.tmpdir(), `xbird-test-${randomUUID()}`);
    ...
    process.env.XBIRD_FEATURES_PATH = cachePath;
