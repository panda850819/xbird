...
const LIVE = process.env.XBIRD_LIVE === '1';
...
const LIVE_NODE_ENV = (process.env.XBIRD_LIVE_NODE_ENV ?? 'production').trim() || 'production';

function runXbird(args: string[], options: { timeoutMs?: number } = {}): Promise<RunResult> {
  if (!LIVE) {
    throw new Error('runXbird() called without XBIRD_LIVE=1');
  }
...
d('live CLI (Twitter/X) all commands', () => {
  const timeoutArg = (process.env.XBIRD_LIVE_TIMEOUT_MS ?? '20000').trim();
  const cookieTimeoutArg = (process.env.XBIRD_LIVE_COOKIE_TIMEOUT_MS ?? '30000').trim();
  ...
  beforeAll(async () => {
    if (!existsSync(CLI_PATH)) {
      throw new Error(`Missing ${CLI_PATH}. Run: bun run build:dist`);
    }

    if (!authToken || !ct0) {
      const check = await runXbird([...baseArgs, '--cookie-timeout', cookieTimeoutArg, 'check'], { timeoutMs: 45_000 });
      if (check.exitCode !== 0) {
        throw new Error(
    ...
    const who = await runXbird([...baseArgs, '--cookie-timeout', cookieTimeoutArg, 'whoami'], { timeoutMs: 45_000 });
    if (who.exitCode !== 0) {
    ...
    const forcedTweetId = (process.env.XBIRD_LIVE_TWEET_ID ?? '').trim();
    if (forcedTweetId) {
      if (!TWEET_ID_REGEX.test(forcedTweetId)) {
        throw new Error(`Invalid XBIRD_LIVE_TWEET_ID (expected digits): "${forcedTweetId}"`);
      }
      tweetId = forcedTweetId;
    } else {
      const searchQuery = (
        process.env.XBIRD_LIVE_SEARCH_QUERY ?? `from:${handle} -filter:replies -filter:retweets`
      ).trim();
      const search = await runXbird(
      ...
      if (!TWEET_ID_REGEX.test(first)) {
        throw new Error(
          `Search returned no usable tweets. Query: "${searchQuery}". ` +
            `Override with XBIRD_LIVE_SEARCH_QUERY.\n${search.stdout}`,
        );
      }
      tweetId = first;
    }

    listId = (process.env.XBIRD_LIVE_LIST_ID ?? '').trim();
    if (!listId) {
      const lists = await runXbird([...baseArgs, '--cookie-timeout', cookieTimeoutArg, 'lists', '-n', '1', '--json'], {
        timeoutMs: 45_000,
      });
      if (lists.exitCode === 0) {
  ...
  it('read returns tweet JSON', async () => {
    const read = await runXbird([...baseArgs, '--cookie-timeout', cookieTimeoutArg, 'read', tweetId, '--json'], {
      timeoutMs: 45_000,
    });
  ...
  it('tweet-id shorthand returns tweet JSON', async () => {
    const shorthand = await runXbird([...baseArgs, '--cookie-timeout', cookieTimeoutArg, tweetId, '--json'], {
      timeoutMs: 45_000,
    });
  ...
  it('replies returns JSON array', async () => {
    const replies = await runXbird([...baseArgs, '--cookie-timeout', cookieTimeoutArg, 'replies', tweetId, '--json'], {
      timeoutMs: 45_000,
    });
  ...
  it('thread returns JSON array', async () => {
    const thread = await runXbird([...baseArgs, '--cookie-timeout', cookieTimeoutArg, 'thread', tweetId, '--json'], {
      timeoutMs: 45_000,
    });
  ...
  it('bookmarks returns JSON array', async () => {
    const bookmarks = await runXbird(
  ...
  it('likes returns JSON array', async () => {
    const likes = await runXbird([...baseArgs, '--cookie-timeout', cookieTimeoutArg, 'likes', '-n', '10', '--json'], {
      timeoutMs: 45_000,
    });
  ...
  it('following returns JSON array', async () => {
    const following = await runXbird(
  ...
  it('followers returns JSON array', async () => {
    const followers = await runXbird(
  ...
  it('user-tweets returns JSON array', async () => {
    const testHandle = process.env.XBIRD_LIVE_USER_TWEETS_HANDLE || 'X';
    const userTweets = await runXbird(
  ...
  it('user-tweets paged JSON returns { tweets, nextCursor }', async () => {
    const testHandle =
      process.env.XBIRD_LIVE_USER_TWEETS_PAGED_HANDLE ?? process.env.XBIRD_LIVE_USER_TWEETS_HANDLE ?? 'X';
    const userTweets = await runXbird(
      [
        ...baseArgs,
        '--cookie-timeout',
  ...
  it('lists returns JSON array', async () => {
    const lists = await runXbird([...baseArgs, '--cookie-timeout', cookieTimeoutArg, 'lists', '-n', '10', '--json'], {
      timeoutMs: 45_000,
    });
  ...
  it('home for-you returns JSON array', async () => {
    const home = await runXbird([...baseArgs, '--cookie-timeout', cookieTimeoutArg, 'home', '-n', '5', '--json'], {
      timeoutMs: 45_000,
    });
  ...
  it('query-ids returns JSON', async () => {
    const queryIds = await runXbird([...baseArgs, '--cookie-timeout', cookieTimeoutArg, 'query-ids', '--json'], {
      timeoutMs: 60_000,
    });
