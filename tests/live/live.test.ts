...
d('live CLI (Twitter/X)', () => {
  ...
  it('bookmarks --folder-id works (opt-in)', async () => {
    const folderId = (process.env.XBIRD_LIVE_BOOKMARK_FOLDER_ID ?? '').trim();
    if (!folderId) {
      return;
    }
    const bookmarks = await runXbird(
  ...
  it('query-ids --fresh works (opt-in)', async () => {
    if (process.env.XBIRD_LIVE_QUERY_IDS_FRESH !== '1') {
      return;
    }
    const queryIds = await runXbird([...baseArgs, 'query-ids', '--fresh', '--json'], { timeoutMs: 5 * 60_000 });
    expect(queryIds.exitCode).toBe(0);
    const snapshot = parseJson<{ cached?: boolean; ids?: Record<string, string> }>(queryIds.stdout);
  ...
  it('long-form tweet (article) extracts rich content (opt-in)', async () => {
    const longformTweetId = (process.env.XBIRD_LIVE_LONGFORM_TWEET_ID ?? '').trim();
    if (!longformTweetId) {
      // Skip unless explicitly provided - long-form tweets may be deleted/unavailable
      return;
    }
    const read = await runXbird(
      [...baseArgs, '--cookie-timeout', cookieTimeoutArg, 'read', longformTweetId, '--json'],
      {
        timeoutMs: 45_000,
      },
    );
